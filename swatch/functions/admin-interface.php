...
<div id="woo-popup-reset" class="woo-save-popup"><div class="woo-save-reset"><?php _e( 'Options Reset', 'woothemes' ); ?></div></div>
    ...
    ?>
        ...
        <form action="<?php /* echo esc_url( $_SERVER['REQUEST_URI'] ); */ ?>" method="post" style="display: inline;" id="wooform-reset">
        <?php
	    	// Add nonce for added security.
...
function woothemes_uploader_function($id,$std,$mod){
	...
	if($mod != 'min') {
            ...
            $uploader .= '<input class="woo-input" name="'. $id .'" id="'. $id .'_upload" type="text" value="'. esc_attr( $val ) .'" />';
	}
